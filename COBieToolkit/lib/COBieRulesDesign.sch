...
<xsl:function name="iso:canComponentBeInTwoSpaces" as="xs:boolean">
	...
	<xsl:function name="iso:uniqueNameMessage" as="xs:string">
		...
		<xsl:variable name="MsgPrefix" as="xs:string" select="iso:assertMsgPrefix($SheetName,$RowKey,$FieldName)"/>
		<xsl:value-of select="concat('Warning:  ',$MsgPrefix,' must exist in ',$FSheet,'.',$FKey)"/>
	</xsl:function>
...
<xsl:function name="iso:validNumber" as="xs:boolean">
	...
	<iso:pattern id="COBieValidation.Errors">
		...
		<iso:rule context="//Types/Type" id="COBie.Types.Type" role="WorksheetErrors">
			...
			<iso:assert id="Type.ModelNumber.NotEmpty" test="iso:validStringOrNA(ModelNumber)" flag="ModelNumber">
		...
		<iso:rule context="//Components/Component" id="COBie.Components.Component" role="WorksheetErrors">
			...
			<iso:assert id="Component.InstallationDate.NotEmpty" test="iso:validStringOrNA(InstallationDate)" flag="InstallationDate">
			...
			<iso:assert id="Component.WarrantyStartDate.NotEmpty" test="iso:validStringOrNA(WarrantyStartDate)" flag="WarrantyStartDate">
		...
		<iso:rule context="//Documents/Document" id="COBie.Documents.Document" role="WorksheetErrors">
			...
			<iso:assert id="Document.Directory.NotEmpty" test="iso:validStringOrNA(Directory)" flag="Directory">
				<value-of select="iso:notEmptyMessage(name(.),@Name,'Directory')"/>
			</iso:assert>
			<iso:report id="Document.File.Check" test="true()" flag="File">File.NotEmpty
			</iso:report>
			<iso:assert id="Document.File.NotEmpty" test="iso:validStringOrNA(File)" flag="File">
				<value-of select="iso:notEmptyMessage(name(.),@Name,'File')"/>
			</iso:assert>
