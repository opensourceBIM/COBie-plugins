...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:altova="http://www.altova.com" xmlns:altovaext="http://www.altova.com/xslt-extensions" xmlns:clitype="clitype" xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:iso4217="http://www.xbrl.org/2003/iso4217" xmlns:ix="http://www.xbrl.org/2008/inlineXBRL" xmlns:java="java" xmlns:link="http://www.xbrl.org/2003/linkbase" xmlns:sps="http://www.altova.com/StyleVision/user-xpath-functions" xmlns:svrl="http://purl.oclc.org/dsdl/svrl" xmlns:xbrldi="http://xbrl.org/2006/xbrldi" xmlns:xbrli="http://www.xbrl.org/2003/instance" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="#all" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns="http://www.w3.org/1999/xhtml">
	...
	<xsl:param name="SV_GeneratedFileNamePrefix" select="if ( $SV_BaseOutputFileName ) then $SV_BaseOutputFileName else &apos;QCwithWarnings&apos;" as="xs:string?"/>
				...
				<body>
					<h1>
						<span style="font-size:xx-large; ">
							<xsl:text>COBie QC report - Construction Deliverable</xsl:text>
						</span>
					</h1>
					<p>
						<span>
							<xsl:variable name="altova:seqContentStrings_16F886A8">
							...
							<xsl:variable name="altova:sContent_16F886A8" select="string($altova:seqContentStrings_16F886A8)"/>
							<xsl:value-of select="$altova:sContent_16F886A8"/>
						</span>
					</p>
...
$failedAssertions"/>
		<altova:level>
			...
			<altova:level>
				...
				<table border="1">
					<xsl:variable name="altova:CurrContextGrid_11689EB0" select="."/>
...
$worksheetName"/>
							<xsl:variable name="successfulReport" as="xs:string" select="following-sibling::svrl:successful-report[1]"/>
							...
							<xsl:for-each select="current-grouping-key()">
								...
								<xsl:variable name="errorCount" as="xs:string" select="if (string($rowCount)!=&apos;NaN&apos; and $rowCount&gt;0) then string(count(distinct-values($failedAsserts[not(sps:isAssertWarning(@id))]/@location))) else &apos;n/a&apos;"/>
								<xsl:variable name="moreThanOneRow" as="xs:boolean" select="$rowCount=$rowCount and $rowCount&gt;0"/>
								<xsl:variable name="warningCount" as="xs:string" select="if (string($rowCount)!=&apos;NaN&apos; and $rowCount&gt;0) then string(count(distinct-values($failedAsserts[sps:isAssertWarning(@id)]/@location))) else &apos;n/a&apos;"/>
								<tr>
									<td>
										<xsl:choose>
			...
			<xsl:for-each-group group-by="preceding-sibling::svrl:fired-rule[1]/@id" select="$errorRoleContent">
				...
				<xsl:variable name="countTotalSheetRows" as="xs:integer" select="count(distinct-values(current-group()/@location))"/>
				<xsl:variable name="countDistinctRowErrors" as="xs:integer" select="count(distinct-values(current-group()[name(self::element())=&apos;svrl:failed-assert&apos; and not(sps:isAssertWarning(@id))]/@location))"/>
				<xsl:variable name="distinctRules" select="distinct-values(current-group()[name(.)=&apos;svrl:successful-report&apos;]/svrl:text)"/>
				...
				<altova:level>
					...
					<p style="text-align:left; ">
						...
						<xsl:for-each select="$percentCorrect">
							<span style="font-style:italic; ">
								<xsl:variable name="altova:seqContentStrings_0F785D48">
									<xsl:value-of select="format-number(number(string(.)), '##0.##')"/>
								</xsl:variable>
								<xsl:variable name="altova:sContent_0F785D48" select="string($altova:seqContentStrings_0F785D48)"/>
								<xsl:value-of select="$altova:sContent_0F785D48"/>
							</span>
							<span style="font-style:italic; ">
								<xsl:text>% correct</xsl:text>
							</span>
					...
					<p style="text-align:left; ">
						<xsl:for-each-group group-by="@flag" select="current-group()">
							<xsl:for-each select="current-grouping-key()">
								<xsl:variable name="incorrectFieldOccurenceCount" as="xs:integer" select="count(distinct-values(current-group()[name(self::element())=&apos;svrl:failed-assert&apos; and not(sps:isAssertWarning(@id))]/@location))"/>
					...
					<p style="text-align:left; ">
						<xsl:for-each-group group-by="@flag" select="current-group()">
							<xsl:for-each select="current-grouping-key()">
								<xsl:variable name="incorrectFieldOccurenceCount" as="xs:integer" select="count(distinct-values(current-group()[name(self::element())=&apos;svrl:failed-assert&apos; and sps:isAssertWarning(@id)]/@location))"/>
								<xsl:variable name="incorrectFieldOccurencesMessage" as="xs:string" select="concat(concat(string(current-grouping-key()),string(&apos;: &apos;)),string($incorrectFieldOccurenceCount))"/>
								<xsl:for-each select="$incorrectFieldOccurencesMessage">
									<xsl:value-of select="."/>
								</xsl:for-each>
		...
		<altova:level>
			...
			<xsl:for-each-group group-by="preceding-sibling::svrl:fired-rule[1]/@id" select="$errorRoleContent[name(.)=&apos;svrl:failed-assert&apos;]">
				<xsl:variable name="ruleGroupName" as="xs:string" select="string(current-grouping-key())"/>
				<xsl:variable name="isWorksheet" as="xs:boolean" select="string-length($ruleGroupName)-string-length(translate($ruleGroupName,&apos;.&apos;,&apos;&apos;))=1"/>
				<xsl:variable name="errorCount" as="xs:integer" select="count(distinct-values(current-group()[not(sps:isAssertWarning(@id))]/@location))"/>
				<xsl:variable name="warningCount" as="xs:integer" select="count(distinct-values(current-group()[sps:isAssertWarning(@id)]/@location))"/>
				<altova:level>
					<altova:marker altova:name="Level1Section">
						<span>
							<xsl:text>&#160;</xsl:text>
					...
					<xsl:for-each-group group-by="@id" select="current-group()">
						<xsl:variable name="formattedLocation" as="xs:string" select="sps:COBieLocationHeading( @location )"/>
						<xsl:variable name="cobieRowName" as="xs:string" select="substring-before(svrl:text ,&apos;:&apos; )"/>
						...
						<altova:level>
							...
							<p style="text-align:left; ">
								<xsl:for-each select="current-group()">
									<xsl:variable name="RowNumber" as="xs:double" select="number(substring-after(sps:COBieLocationHeading(current()/@location),&apos; &apos;))+1"/>
									<altova:hyperlink>
										<xsl:choose>
											<xsl:when test="not(string(concat(&apos;#&apos;,@location)))">
												<xsl:attribute name="href">
													<xsl:text>#</xsl:text>
												</xsl:attribute>
											</xsl:when>
											<xsl:when test="substring(string(concat(&apos;#&apos;,@location)), 1, 1) = '#'">
												<xsl:attribute name="href">
											...
											<xsl:otherwise>
												<xsl:attribute name="href">
													<xsl:if test="substring(string(concat(&apos;#&apos;,@location)), 2, 1) = ':'">
														<xsl:text>file:///</xsl:text>
													</xsl:if>
													<xsl:value-of select="translate(string(concat(&apos;#&apos;,@location)), '&#x5c;', '/')"/>
												</xsl:attribute>
											</xsl:otherwise>
										</xsl:choose>
		...
		<altova:level>
			...
			<xsl:for-each-group group-by="preceding-sibling::svrl:fired-rule[1]/@id" select="$errorRoleContent[name(.)=&apos;svrl:failed-assert&apos;]">
				...
				<altova:level>
					...
					<xsl:for-each-group group-by="@location" select="current-group()">
						<xsl:variable name="formattedLocation" as="xs:string" select="sps:COBieLocationHeading( @location )"/>
						<xsl:variable name="cobieRowName" as="xs:string" select="substring-before(svrl:text ,&apos;:&apos; )"/>
						<xsl:variable name="cobieRowNameNoPrefix" as="xs:string" select="substring-after($cobieRowName,&apos;.&apos;)"/>
						<xsl:variable name="locationPrefix" as="xs:string" select="substring-before($formattedLocation,&apos; &apos;)"/>
...
concat(sps:singularizeSheetName($locationPrefix),&apos;[*]&apos;)"/>
						...
						<altova:level>
							...
							<xsl:if test="count(current-group()[not(sps:isAssertWarning(@id))])&gt;0">
								<table border="1">
									<xsl:variable name="altova:CurrContextGrid_12CF46B8" select="."/>
									<xsl:variable name="altova:ColumnData">
										<altova:Internal/>
									</xsl:variable>
									<thead>
							...
							<xsl:if test="count(current-group()[sps:isAssertWarning(@id)])&gt;0">
								<table border="1">
									<xsl:variable name="altova:CurrContextGrid_13BA3668" select="."/>
									<xsl:variable name="altova:ColumnData">
										<altova:Internal/>
									</xsl:variable>
									<thead>
	...
	<xsl:function name="sps:COBieLocationHeading" as="xs:string">
		<xsl:param name="cobiePathString" as="xs:string"/>
		<xsl:sequence select="replace(replace(string(tokenize($cobiePathString,&apos;/&apos;)[last()]),&apos;\[&apos;,&apos; &apos;),&apos;\]&apos;,&apos;&apos;)"/>
	</xsl:function>
