	...
	<xsl:variable name="altova:DesignAfterStep1">
		<altova:main-document url="{if ($SV_BaseOutputFileName) then concat($SV_BaseOutputFileName,'.xhtml') else ''}">
			<html>
				<head>
					<title/>
					<meta name="generator" content="Altova StyleVision Enterprise Edition 2013 rel. 2 sp2 (x64) (http://www.altova.com)"/>
 ...
 h5 { font-size:medium;
				...
				<body>
					...
					<p style="text-align:justify; ">
						<span>
							<xsl:text>Save a new copy of your COBie spreadsheet to a new file. The errors in this report will match by Worksheet Name, Row Number, and object name to the data in your file.&#160; For example, a problem with the fifth space, named &quot;Vestibule,&quot; willbe listed as Space[5]:Vestibule.&#160; Once each relevant issue is fixed, re-run your file. </xsl:text>
						</span>
					</p>
							...
							<xsl:for-each select="current-grouping-key()">
								...
								<tr>
									...
									<td>
										<altova:hyperlink>
											<xsl:choose>
												<xsl:when test="not(string(concat(&apos;#count&apos;,string($worksheetName))))">
													<xsl:attribute name="href">
														<xsl:text>#</xsl:text>
													</xsl:attribute>
												</xsl:when>
												<xsl:when test="substring(string(concat(&apos;#count&apos;,string($worksheetName))), 1, 1) = '#'">
													<xsl:attribute name="href">
														<xsl:value-of select="concat(&apos;#count&apos;,string($worksheetName))"/>
													</xsl:attribute>
												</xsl:when>
												<xsl:otherwise>
													<xsl:attribute name="href">
														<xsl:if test="substring(string(concat(&apos;#count&apos;,string($worksheetName))), 2, 1) = ':'">
															<xsl:text>file:///</xsl:text>
														</xsl:if>
														<xsl:value-of select="translate(string(concat(&apos;#count&apos;,string($worksheetName))), '&#x5c;', '/')"/>
													</xsl:attribute>
												</xsl:otherwise>
											</xsl:choose>
									...
									<td>
										<altova:hyperlink>
											<xsl:choose>
												<xsl:when test="not(string(concat(&apos;#details&apos;,string( $rootMatch ))))">
													<xsl:attribute name="href">
														<xsl:text>#</xsl:text>
													</xsl:attribute>
												</xsl:when>
												<xsl:when test="substring(string(concat(&apos;#details&apos;,string( $rootMatch ))), 1, 1) = '#'">
													<xsl:attribute name="href">
														<xsl:value-of select="concat(&apos;#details&apos;,string( $rootMatch ))"/>
													</xsl:attribute>
												</xsl:when>
												<xsl:otherwise>
													<xsl:attribute name="href">
														<xsl:if test="substring(string(concat(&apos;#details&apos;,string( $rootMatch ))), 2, 1) = ':'">
															<xsl:text>file:///</xsl:text>
														</xsl:if>
														<xsl:value-of select="translate(string(concat(&apos;#details&apos;,string( $rootMatch ))), '&#x5c;', '/')"/>
													</xsl:attribute>
												</xsl:otherwise>
											</xsl:choose>
								...
								<xsl:for-each select="$ruleGroupName">
									...
									<xsl:choose>
										<xsl:when test="$isWorksheet">
											<altova:bookmark>
												<xsl:attribute name="name">
													<xsl:value-of select="concat(&apos;count&apos;,substring-after($ruleGroupName, &apos;COBie.&apos;))"/>
												</xsl:attribute>
												<span>
			...
			<xsl:for-each-group group-by="preceding-sibling::svrl:fired-rule[1]/@id" select="$errorRoleContent[name(.)=&apos;svrl:failed-assert&apos;]">
				...
				<altova:level>
					<altova:marker altova:name="Level1Section">
						...
						<altova:bookmark>
							<xsl:attribute name="name">
								<xsl:value-of select="concat(&apos;details&apos;,encode-for-uri($ruleGroupName))"/>
							</xsl:attribute>
							<h4>
								<xsl:for-each select="$ruleGroupName">
									<span>
