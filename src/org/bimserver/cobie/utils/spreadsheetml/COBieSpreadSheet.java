...
public class COBieSpreadSheet {
 ...
 ////functions pulled from generic sheet tool
	...
	private ArrayList<String> distinctCategoryValuesFromSheetName(CobieSheetName sheetName)
	{
		...
		try
		{
			ArrayList<String> sheetColNames = COBieUtility.getCobieSheetNameColumnMappings().get(sheetName);
			Map<String,Integer> dictionaryMap = COBieUtility.GetWorksheetColumnDictionary(targetSheet, sheetColNames);
	...
	public void populatePickListCategoryValues(String pickListColumnName,ArrayList<String> categoryValues,Worksheet pickListSheet)
	{
		...
		Map<String,Integer> pickListMap = COBieUtility.GetWorksheetColumnDictionary(pickListSheet, pickListColumnNames);
