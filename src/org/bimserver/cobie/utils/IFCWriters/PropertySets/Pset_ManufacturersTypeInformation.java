...
{
	private static final String pSetName = "Pset_ManufacturersTypeInformation";
	private static final String pSetDescription =
			"Defines characteristics of manufactured products that " +
			"may be given by the manufacturer. Note that the term may " +
			"also be used to refer to products that are supplied and " +
			"identified by the supplier or that are assembled off site " +
	...
	public Pset_ManufacturersTypeInformation(TypeType cobieType)
	{
		...
		if (!COBieUtility.isNA(manufacturer) && ! manufacturer.equalsIgnoreCase(manufacturerPropertyName))
			this.setManufacturer(cobieType.getManufacturer());
		String modelNum = cobieType.getModelNumber();
		
		this.setModelLabel(cobieType.getModelNumber());
		this.setModelReference(cobieType.getModelReference());
	}
