...
{
	protected static final int COBieAllowedValuesSplitStringCountLowerBound = 1;
	protected static final int COBieBoundedValueSplitStringCountLowerBound = 2;
	protected static final int BoundedValueLowerBoundIdx = 0;
	protected static final int BoundedValueUpperBoundIdx = 1;
	protected static final String AllowedValueExceptionMsg = "Invalid allowed values";
	protected static final String EnumerationNameSuffix = " Enumeration";
	protected static final String InvalidValueExceptionMsg = "Invalid value:  value is not in list of allowed values";
	
	public static IfcProperty propertyFromCOBieString(String cobieString,String propertyName,String propertyDescription)
	{
		boolean doubleParsed = false;
		boolean delimitedValuesParsed = true;
		double dblValue;
		String delim = COBieUtility.getCOBieDelim();
		String[] splitStrings = cobieString.split(delim);
		boolean candidateSplitString = (splitStrings!=null && splitStrings.length==COBieBoundedValueSplitStringCountLowerBound);
		try
		{
			dblValue = Double.parseDouble(cobieString);
	...
	{
		String exceptionMsg = AllowedValueExceptionMsg;
		...
		enumeration.setName(propertyName + EnumerationNameSuffix);
		...
		ArrayList<String> allowedValsList = (ArrayList<String>) Arrays.asList(splitAllowedVals);
		try
		{
			...
			String valueString,ArrayList<String> allowedValues) throws Exception
	{
		...
		Exception valException =
				new Exception(PropertyUtility.InvalidValueExceptionMsg);
		...
		else
		{
			if (splitVals!=null && splitVals.length>1)
			{
				ArrayList<String> valList = 
						(ArrayList<String>) Arrays.asList(splitVals);
	...
	private static void populateEnumeration(IfcPropertyEnumeration enumeration, String[] valueStrings) throws Exception
	{
		if (valueStrings!=null && valueStrings.length > COBieAllowedValuesSplitStringCountLowerBound)
		{
			...
			throw new Exception(PropertyUtility.AllowedValueExceptionMsg);
	}
	...
	public static boolean isCandidateEnumeration(String valueString,String allowedValString)
	{
		...
		boolean allowedValuesValid = 
				...
				COBieAllowedValuesSplitStringCountLowerBound);
		boolean isCandidateEnum = false;
		boolean valueStringInEnum;
		if (allowedValuesValid)
		{
			ArrayList<String> allowedValsList = (ArrayList<String>) Arrays.asList(allowedValsSplit);
	...
	private static boolean allStringsAreNumbers(String[] stringArray)
	{
		...
		for(String tmpString : stringArray)
		{
			try
			{
				tmpDbl = Double.parseDouble(tmpString);
			}
			catch(Exception e)
			{
				allNumbers = false;
