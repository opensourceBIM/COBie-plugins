...
concat(upper-case(substring($x, 1, 1)), lower-case(substring($x, 2))), ' ')"/>
	...
	<iso:pattern id="COBieValidation.General">
		...
		<iso:rule context="//Facilities/Facility" id="COBie.Facilities.Facility" role="Errors">
			...
			<!-- TODO Insert Picklist Category assert -->
		...
		<iso:rule context="//Floors/Floor" id="COBie.Floors.Floor" role="Errors">
			...
			<iso:extends rule="COBie.Abstract.Category"/>
			<!-- TODO Add Picklist check for Category -->
			<iso:assert id="Floor.Elevation.ZeroOrGreater" test="iso:validNumberZeroOrGreater(Elevation)">
		...
		<iso:rule context="//Spaces/Space" id="COBie.Spaces.Space" role="Errors">
			...
			<iso:assert id="Space.GrossArea" test="iso:validNumberNotZero(GrossArea)">
				<value-of select="iso:notEmptyNumberMessage(name(.),@Name,'GrossArea')"/>
			</iso:assert>
			<iso:assert id="Space.NetArea" test="iso:validNumberNotZero(NetArea)">
		...
		<iso:rule context="//Types/Type" id="COBie.Types.Type" role="Errors">
			...
			<iso:assert id="Type.Size.ZeroOrGreater" test="iso:validNumberZeroOrGreater(Size)">
				<value-of select="iso:notEmptyNumberMessage(name(.),@Name,'Size')"/>
			</iso:assert>
			...
			<iso:assert id="Type.SustainabilitPerformance.NotEmpty" test="iso:validStringOrNA(SustainabilitPerformance)">
				<value-of select="iso:notEmptyMessage(name(.),@Name,'SustainabilitPerformance')"/>
			</iso:assert>
		</iso:rule>
