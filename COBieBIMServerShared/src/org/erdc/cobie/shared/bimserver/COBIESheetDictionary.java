...
import org.erdc.cobie.shared.COBieNASetter;
...
public class COBIESheetDictionary extends HashMap<String, COBIESheetCollection>
{
    /**
	 * 
	 */
    ...
    {
        HashMap<String, Node> columnNameToXMLNode = new HashMap<String, Node>();
        ...
        for (String columnName : rowData.keySet())
        {
            ...
            if (domNode.getChildNodes() != null)
            {
                for (int i = 0; i < domNode.getChildNodes().getLength(); i++)
                {
                    if (!foundNodeMatch)
                    {
                        Node elementNode = domNode.getChildNodes().item(i);
    ...
    {
        setSheetNameColumnValMap(cobieDocument);
    ...
    public COBIEDocument asCOBie() throws Exception
    {
        ...
        {
            XmlObject xmlCOBieSheet = getCOBieSheetXMLObject(cobie, sheetName);
            if (xmlCOBieSheet != null)
            {
                COBIESheetCollection cobieSheetCollection = get(sheetName);
                for (COBIERowDictionary cobieRow : cobieSheetCollection)
                {
                    try
                    {
                        XmlObject rowXmlObject = getNewSheetRowXMLObject(xmlCOBieSheet);
    ...
    public void deleteRow(String sheetName, String rowKey)
    {
        ...
        {
            COBIESheetCollection targetSheet = get(sheetName);
            COBIERowDictionary targetRow = targetSheet.getByCOBieRowIdString(rowKey);
    ...
    {
        COBieUtility.CobieSheetName cobieSheetName = COBieUtility.CobieSheetName.valueOf(sheetName);
    ...
    {
        return COBieNASetter.initializeCOBieElementToNA(xmlSheetObject);
    }
    ...
    private void handleAddNewCOBieRow(String tempMasterKey, COBIESheetCollection tmpArray, COBIERowDictionary tmpElementMap)
    {
        tmpElementMap.setSheetName(tempMasterKey);
        tmpArray.add(tmpElementMap);
        if (!tmpElementMap.isGuidEmpty())
        {
            guidToSheetName.put(tmpElementMap.getGuid(), tempMasterKey);
        }
    }
    ...
    {
        // if there is no data available then the try catch will handle the
        ...
        try
        {
            ...
            if ((contacts != null) && !contacts.isNil())
            {
                ...
                put(tempMasterKey, (COBIESheetCollection)tmpArray.clone());
            }
    ...
    private void setXMLObjectValuesFromRowDictionary(XmlObject rowXmlObject, COBIERowDictionary rowData)
    {
        Node domNode = rowXmlObject.getDomNode();
        HashMap<String, Node> columnNameToXMLNode = getColumnNameNodeMap(rowData, domNode);
        ...
        /*
         * try { rowXmlObject.set(XmlObject.Factory.parse(domNode)); } catch
