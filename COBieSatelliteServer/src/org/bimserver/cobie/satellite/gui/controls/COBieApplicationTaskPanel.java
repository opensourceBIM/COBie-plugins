...
import org.bimserver.cobie.satellite.tasks.Informable;
...
//		progressTextArea.updateUI();
	...
	private void defaultCancelActions()
	{
		if(Thread.currentThread().getState()!=Thread.State.TIMED_WAITING)
			takeAShortNap();
		taskComponents.enableAllControls();
		if(primaryTask instanceof BimServerTaskConnectable)
		{
			handleBiMServerTask();
		}
		hideProgressControls();
	...
	private void handleBiMServerTask()
	{
		BimServerTaskConnectable satelliteTask =
				(BimServerTaskConnectable) primaryTask;
		
		try
		{
			if(primaryTask.isRequiresRunningBiMServer() &&
					!satelliteTask.getSatelliteServer().allServicesConnected())	
			{
				taskComponents.disableAllControls();
			}
		}
