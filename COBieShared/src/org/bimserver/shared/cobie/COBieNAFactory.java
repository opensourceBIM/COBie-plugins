...
{
	public static XmlObject initializeCOBieElementToNA(
			XmlObject pluralCobieObject) throws Exception
	{
		String nodeName = pluralCobieObject.getDomNode().getNodeName();
		CobieSheetName sheetName = COBieUtility.CobiePluralNameToCobieSheetName
				.get(nodeName);
		XmlObject newObject = null;
		if (sheetName != null)
		{
			newObject = getEmptyXmlObjectFromSheetName(pluralCobieObject,
					sheetName);
	...
	private static XmlObject getEmptyXmlObjectFromSheetName(
			XmlObject pluralCobieObject, CobieSheetName sheetName)
	{
		XmlObject newObject = null;
		switch (sheetName)
		{
		case Assembly:
			newObject = ((COBIEType.Assemblies) pluralCobieObject)
					.addNewAssembly();
			break;
		case Attribute:
			newObject = ((COBIEType.Attributes) pluralCobieObject)
					.addNewAttribute();
			break;
		case Component:
			newObject = ((COBIEType.Components) pluralCobieObject)
					.addNewComponent();
			break;
		case Connection:
			newObject = ((COBIEType.Connections) pluralCobieObject)
					.addNewConnection();
			break;
		case Contact:
			newObject = ((COBIEType.Contacts) pluralCobieObject)
					.addNewContact();
			break;
		case Coordinate:
			newObject = ((COBIEType.Coordinates) pluralCobieObject)
					.addNewCoordinate();
			break;
		case Document:
			newObject = ((COBIEType.Documents) pluralCobieObject)
					.addNewDocument();
			break;
		case Facility:
			newObject = ((COBIEType.Facilities) pluralCobieObject)
					.addNewFacility();
			break;
		case Floor:
		...
		case Job:
			...
			break;
		case Resource:
			newObject = ((COBIEType.Resources) pluralCobieObject)
					.addNewResource();
			break;
		case Space:
	...
	private static void setXmlObjectToNA(XmlObject xmlObject)
	{
		if (xmlObject instanceof AssemblyType)
		{
			...
			if (xmlObject instanceof AttributeType)
			{
				setAttributeNA((AttributeType)xmlObject);
			}
			else
				if (xmlObject instanceof ComponentType)
				{
					setComponentNA((ComponentType)xmlObject);
				}
				else
					if (xmlObject instanceof ConnectionType)
					{
						setConnectionNA((ConnectionType)xmlObject);
					}
					else
						if (xmlObject instanceof ContactType)
						{
							setContactNA((ContactType)xmlObject);
						}
						else
							if (xmlObject instanceof CoordinateType)
							{
								setCoordinateNA((CoordinateType)xmlObject);
							}
							else
								if (xmlObject instanceof DocumentType)
								{
									setDocumentNA((DocumentType)xmlObject);
								}
								else
									if (xmlObject instanceof FacilityType)
									{
										setFacilityNA((FacilityType)xmlObject);
									}
									else
										if (xmlObject instanceof FloorType)
										{
											setFloorNA((FloorType)xmlObject);
										}
										else
											if (xmlObject instanceof ImpactType)
											{
												setImpactNA((ImpactType)xmlObject);
											}
											else
												if (xmlObject instanceof IssueType)
												{
													setIssueNA((IssueType)xmlObject);
												}
												else
													if (xmlObject instanceof JobType)
													{
														setJobNA((JobType)xmlObject);
													}
													else
														if (xmlObject instanceof ResourceType)
														{
															setResourceNA((ResourceType)xmlObject);
														}
														else
															if (xmlObject instanceof SpaceType)
															{
																setSpaceNA((SpaceType)xmlObject);
															}
															else
																if (xmlObject instanceof SpareType)
																{
																	...
																	if (xmlObject instanceof SystemType)
																	{
																		...
																		if (xmlObject instanceof TypeType)
																		{
																			setTypeNA((TypeType)xmlObject);
																		}
																		else
																			if (xmlObject instanceof ZoneType)
																			{
