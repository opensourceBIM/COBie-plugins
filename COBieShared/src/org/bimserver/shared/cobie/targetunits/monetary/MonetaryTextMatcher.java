package org.bimserver.shared.cobie.targetunits.monetary;
...
public class MonetaryTextMatcher
{
	...
	private double getBestMatchSecondaryDistance()
	{
		return bestMatchSecondaryDistance;
	}
		...
		for (IfcCurrencyEnum currencyLiteral : IfcCurrencyEnum.values())
		{
			...
			case DDP:
				currencyToKeywords.put(IfcCurrencyEnum.DDP, new String[]
					{
							"Dominican Republic", "Dominican peso", "DDP",
							"Dominican pesos"
					});
				break;
			case EGL:
			...
			case XEU:
				currencyToKeywords.put(IfcCurrencyEnum.XEU, new String[]
					{
							"Europe (States of the European Union)", "XEU",
							"XEUs"
					});
				break;
			case EUR:
				...
				currencyToKeywords.put(IfcCurrencyEnum.USD, new String[]
					{
							...
							"USD", "United States dollars","dollar","dollars"
					});
				break;
			case VEB:
	...
	private void initializeSearchResults()
	{
		...
		for (IfcCurrencyEnum currency : IfcCurrencyEnum.VALUES)
		{
			...
			currencyToShortestKeywordSearchDistance.put(currency,
					localShortestDistance);
			currencyToMatchedKeyword.put(currency, localMatchedKeyword);
			evaluateAgainstBestMatch(currency, localShortestDistance,
					localMatchedKeyword);
		}
	}

	private void evaluateAgainstBestMatch(IfcCurrencyEnum currency,
			double localShortestDistance, String localMatchedKeyword)
	{
		if (localShortestDistance < getMatchRank())
		{
			...
			if (localShortestDistance == getMatchRank())
			{
				double tmpSecondaryDistance = getSecondaryDistance(searchString,
						localMatchedKeyword);
				if (tmpSecondaryDistance < bestMatchSecondaryDistance)
				{
					setBestMatch(currency, localShortestDistance,
							localMatchedKeyword);
				}
			}
	}

	private void setBestMatch(IfcCurrencyEnum currency,
			double localShortestDistance, String localMatchedKeyword)
	{
		setMatchRank(localShortestDistance);
		setBestKeywordMatch(localMatchedKeyword);
		setMatchingCurrency(currency);
		setBestMatchSecondaryDistance(getSecondaryDistance(searchString,
				localMatchedKeyword));
	}
	...
	{
		return StringDistanceCalculator.getEditDistance(searchString,
				keywordTarget, distanceAlgorithm);
	}

	private double getSecondaryDistance(String searchString, String keywordTarget)
	{
		return StringDistanceCalculator.getEditDistance(searchString,
				keywordTarget, secondaryDistanceAlgorithm);
	}
	...
	public void setCurrencyToKeywords(
			HashMap<IfcCurrencyEnum, String[]> currencyToKeywords)
	{
	...
	private void setCurrencyToLevenshteinDistance(
			HashMap<IfcCurrencyEnum, Double> currencyToLevenshteinDistance)
	{
		this.currencyToShortestKeywordSearchDistance = currencyToLevenshteinDistance;
	}

	private void setCurrencyToMatchedKeyword(
			HashMap<IfcCurrencyEnum, String> currencyToMatchedKeyword)
	{
		this.currencyToMatchedKeyword = currencyToMatchedKeyword;
	}

	private void setCurrencyToShortestKeywordSearchDistance(
			HashMap<IfcCurrencyEnum,Double> currencyToShortestKeywordSearchDistance)
	{
