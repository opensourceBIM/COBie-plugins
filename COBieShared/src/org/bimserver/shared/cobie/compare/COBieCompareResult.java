...
import org.bimserver.shared.cobie.compare.COBieCompareState.CompareMode;
import org.bimserver.shared.cobie.compare.COBieCompareState.ComparisonDocument;
...
public class COBieCompareResult//extends SCompareResult
{
	...
	public COBieCompareResult(long roid1,long roid2)
	{
		initializeState();
		this.roid1 = roid1;
		this.roid2 = roid2;
	...
	public COBieCompareResult(COBIEDocument baselineDocument,COBIEDocument revisionDocument) 
	{
		/*try
		{
			d.evaluateXQuery(" for $c in //Components/Component where lower-case($c//ExtObject)='ifcdoor' return $c ");
		}
		catch (XQException e)
		{
	...
	public COBieCompareResult(long roid1, long roid2,ServiceInterface service) 
	...
	public void initializeBiMServerCompare(long roid1, long roid2,ServiceInterface service) 
			throws ServerException, UserException, XmlException, IOException
	{
		//initializeState();
		this.roid1 = roid1;
	...
	public  COBieCompareDocument asCOBieCompareDocument() throws ServerException, UserException
	{
		...
		int progressLeft = 100-progressBefore;
		...
		for(String sheetName:this.sheetNameToCobieCompareItems.keySet())
		{
			try
			{
			...
			handleCompareDocumentAddSheetComparisons(compareDocument.getCOBieCompare(),
					sheetName);
	...
	public void compareSheet(String sheetName)
	{
		if(cobie1Lookup.containsKey(sheetName) && cobie2Lookup.containsKey(sheetName))
		{
			COBIESheetCollection cobie1Sheet =
					cobie1Lookup.get(sheetName);
			COBIESheetCollection cobie2Sheet =
					cobie2Lookup.get(sheetName);
			ArrayList<String> document1HashesProcessed =
					new ArrayList<String>();
			for(COBIERowDictionary cobie1Row: cobie1Sheet)
			{
				String cobie1RowIDHash =
						cobie1Row.getCOBieRowIdHash();
					COBIERowDictionary cobie2Row =
							cobie2Sheet.getByRowHash(cobie1RowIDHash);
					if (cobie2Row==null)
						handleDeletedRow(cobie1RowIDHash,sheetName);
					else
						handleModifiedRow(cobie1Row,
							cobie1RowIDHash, cobie2Row);
				document1HashesProcessed.add(cobie1RowIDHash);
			}
			for(COBIERowDictionary cobie2Row: cobie2Sheet)
			{
				String cobie2RowIDHash =
						cobie2Row.getCOBieRowIdHash();
	...
	public void handleAddedRow(String rowHash,
			COBIERowDictionary newCOBieRow)
	{
		String sheetName = newCOBieRow.getSheetName();
		COBieCompareItemAdd addItem =
				new COBieCompareItemAdd(cobie2Lookup,rowHash,sheetName);
		handleAddNewCompareItem(sheetName, addItem);
	}
	
	public void handleAddNewCompareItem(String sheetName,
			COBieCompareItem compareItem)
	{
		ArrayList<COBieCompareItem> compareItems;
		if (sheetNameToCobieCompareItems.containsKey(sheetName))
		{
		 compareItems =
				this.sheetNameToCobieCompareItems.get(sheetName);
		}
		else
			compareItems = new ArrayList<COBieCompareItem>();
		compareItems.add(compareItem);
		this.sheetNameToCobieCompareItems.put(sheetName,compareItems);
	}
	


	public void handleCompareDocumentAddSheetComparisons(
			COBieCompareType compareDocumentRoot, String sheetName)
	{
		...
		for(COBieCompareItem compareItem: sheetNameToCobieCompareItems.get(sheetName))
		{
			...
			{
				COBieItemAddedType itemAdded =
						itemsAdded.addNewCOBieItemAdded();
			...
			{
				COBieItemDeletedType itemDeleted =
						itemsDeleted.addNewCOBieItemDeleted();
				COBieCompareItemDelete tmpDeletedItem =
						(COBieCompareItemDelete) compareItem;
			...
			{
				COBieCompareItemModify compareItemModify =
						(COBieCompareItemModify) compareItem;
				COBieItemModifiedType itemModified =
				itemsModified.addNewCOBieItemModified();
				...
				{
					COBieItemModifiedChangeType change =
							changes.addNewCOBieChangeItem();
	...
	{
		COBieCompareItemDelete deleteItem =
				new COBieCompareItemDelete(cobie1Lookup,rowHash,sheetName);
		handleAddNewCompareItem(sheetName,deleteItem);
	}
	
	public void handleModifiedRow(COBIERowDictionary cobie1Row, String rowHash,
			COBIERowDictionary cobie2Row)
	{
		...
		String cobie2RowValuesHash =
				cobie2Row.getRowValuesHash();
		...
		{
			COBieCompareItemModify modifyItem =
					new COBieCompareItemModify(cobie1Lookup,cobie2Lookup,rowHash,sheetName);
	...
	private void indexDocuments()
	{
		...
		double decimalProgressAfterRevision = (double) 1.0;
	...
	private COBieCompareDocument initializeCOBieCompareDocumentWithBiMServerMetadata() throws ServerException, UserException
	{
		COBieCompareDocument compareDocument =
				COBieCompareDocument.Factory.newInstance();
		COBieCompareType compareDocumentRoot =
				compareDocument.addNewCOBieCompare();
		CompareMetadataType compareMetadata =
				compareDocumentRoot.addNewCompareMetadata();
	...
	private void performComparisons()
	{
		...
		this.sheetNameToCobieCompareItems = new HashMap<String,ArrayList<COBieCompareItem>>();
		int sheetCount = cobie1Lookup.keySet().size();
		int loopCount = 0;
		int currentProgress;
		int progressBefore = state.getProgress();
	...
	private int getCummulativePercentDone(int progressBefore,int maxProgress,
			double decimalPercentDoneSubtask)
	{
		int currentProgress;
