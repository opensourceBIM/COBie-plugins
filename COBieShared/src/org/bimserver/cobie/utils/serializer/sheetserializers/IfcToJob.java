...
public class IfcToJob 
{
	...
	private static final ArrayList<String> _DURATION_PROPERTY_NAMES =
			new ArrayList<String>(Arrays.asList(TASK_DURATION_PROPERTY_NAME));
	...
	private static String priorsFromTask(IfcTask task)
	{
		...
		for(IfcRelSequence relSequence :task.getIsSuccessorFrom())
		{
			...
			{
				IfcTask priorTask =
						(IfcTask) relSequence.getRelatingProcess();
				String priorTaskId =
						priorTask.getTaskId();
