...
public class IfcToZone 
{
	...
	private static void zonesToCOBieZonesSpacePerRow(
		...
		for (IfcZone zone : zones) 
		{
			processCandidateCOBieZone(loggerHandler, cZones, zoneNamesAdded,
					zone);	
		}
	}

	private static void processCandidateCOBieZone(LogHandler loggerHandler,
			COBIEType.Zones cZones, ArrayList<String> zoneNamesAdded,
			IfcZone zone)
	{
		IfcOwnerHistory oh;
		String name;
	...
	{
		String category = "";
		category = COBieUtility.getObjectClassificationCategoryString(zone);
		return category;
	}
