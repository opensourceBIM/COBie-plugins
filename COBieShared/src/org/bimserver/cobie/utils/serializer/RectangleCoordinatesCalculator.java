...
{
	public enum RectangleCoordinateSourceEnum {boundingbox,sweptareasolid}
	...
	private Vector<Double> calculateSweptAreaSolidLowerLeft()
	{
		...
		//iterate through x and y coordinates, skip z b/c it is only location + position z
	...
	{
		Vector<Double> representationPoint =
				new Vector<Double>();
		...
		if(productRepresentation instanceof IfcProductDefinitionShape)
		{
			IfcProductDefinitionShape definitionShape =
					(IfcProductDefinitionShape) productRepresentation;
			setVectorsFromProductDefinitionShape(definitionShape);
		}
	}

	private void setVectorsFromProductDefinitionShape(
			IfcProductDefinitionShape definitionShape)
	{
		...
		for(IfcRepresentation rep:definitionShape.getRepresentations())
		{
			for(IfcRepresentationItem repItem:rep.getItems())
			{
				...
				if(repItem instanceof IfcBoundingBox && !foundFirstShapeRepresentation)
				{
					...
					setBoundingBoxVector(boundingBoxPointVectorFromRepresentationItem((IfcBoundingBox)repItem));
				...
				if(repItem instanceof IfcSweptAreaSolid && !foundFirstShapeRepresentation)
				{
					...
					if(sweptAreaSolid.getSweptArea() instanceof IfcRectangleProfileDef)
					{
						...
						IfcRectangleProfileDef rectangleProfile =
								(IfcRectangleProfileDef)sweptAreaSolid.getSweptArea();
	...
	private Vector<Double> positionVectorFromRectangleProfile(
			IfcRectangleProfileDef rectangleProfile)
	{
		IfcCartesianPoint positionPoint =
				rectangleProfile.getPosition().getLocation();
	...
	private Vector<Double> getSweptAreaSolidVector(IfcSweptAreaSolid sweptAreaSolid,
			IfcRectangleProfileDef rectangleProfile)
	{
		Vector<Double> representationPoint =
				new Vector<Double>();
	...
	public Vector<Double> locationVectorFromProduct(
			IfcProduct product)
	{
	...
	private void setRectangleCoordinateSource(
			RectangleCoordinateSourceEnum rectangleCoordinateSource)
	{
		this.rectangleCoordinateSource = rectangleCoordinateSource;
	}
	
	private void setRectangleProfilePositionVector(
			Vector<Double> rectangleProfilePositionVector)
	{
	...
	{
		return (lowerLeftVector.size()==THREE_D_DIMENSION_COUNT && 
				upperRightVector.size()==THREE_D_DIMENSION_COUNT);
	}
}
