...
import org.bimserver.shared.cobie.CaseAgnosticStringList;
import org.bimserver.shared.cobie.targetunits.IfcUnitSearchDispatcher;
...
public class PropertyUtility 
{
	...
	{
		IfcTimeMeasure valReal =
				Ifc2x3tc1Factory.eINSTANCE.createIfcTimeMeasure();
		float valFloat = 0;
		String valueAsString = null;
		try
		{
	...
	{
		IfcInteger ifcValInteger =
				Ifc2x3tc1Factory.eINSTANCE.createIfcInteger();
		int valInt = 0;
		String valueAsString = null;
		try
		{
	...
	public static void popoulateEnumerationValues(IfcPropertyEnumeratedValue enumValue, 
			String valueString,ArrayList<String> allowedValues) throws Exception
	{
		CaseAgnosticStringList allowedValuesCaseAgnostic =
				new CaseAgnosticStringList(allowedValues);
		...
		if (allowedValuesCaseAgnostic.contains(valueString))
		{
			String caseSensitiveValue =
					allowedValuesCaseAgnostic.getOriginalCaseSensitiveValue(valueString);
		...
		else
		{
			if (splitVals!=null && splitVals.length>1)
			{
				...
				if (allowedValuesCaseAgnostic.containsAll(valList))
				{
					...
					{
						String caseSensitiveValue =
								allowedValuesCaseAgnostic.getOriginalCaseSensitiveValue(splitStr);
						enumValue.getEnumerationValues().add(valueFromString(caseSensitiveValue));
					}
				}
				else
	...
	{
		ArrayList<String> allowedValsList =
				new ArrayList<String>(Arrays.asList(allowedValues.split(COBieUtility.getCOBieDelim())));
		ArrayList<String> trimmedAllowedValsList = 
	...
	public static boolean isCandidateEnumeration(String valueString,String allowedValString)
	{
		...
		/*** commented out 8/3/2012..need to allow enums regardless of value
		 * if (allowedValuesValid)
		{
			ArrayList<String> allowedValsList =  allowedValuesDelimittedToArrayList(allowedValString);
			CaseAgnosticStringList caseAgnosticList = new CaseAgnosticStringList(allowedValsList);
			
			String[] valueSplit = valueString.split(COBieUtility.getCOBieDelim());
			valueStringInEnum = caseAgnosticList.contains(valueString);
			...
			else if (valueSplit!=null && valueSplit.length>1)
			{
				...
				{
					if (!(caseAgnosticList.contains(tmpStr)))
	...
	protected static void setIfcPhysicalSimpleQuantityValue(IfcPhysicalSimpleQuantity quantity,
			IfcReal ifcReal)
	{
		boolean setAsString = (ifcReal.getWrappedValueAsString()!=null && 
				ifcReal.getWrappedValueAsString().equals(COBieUtility.COBieNA));
	...
	protected static void setIfcPhysicalSimpleQuantityValueAndUnit(IfcPhysicalSimpleQuantity quantity,
			IfcReal ifcReal,IfcNamedUnit unit)
	{
		boolean setAsString = (ifcReal.getWrappedValueAsString()!=null && 
				ifcReal.getWrappedValueAsString().equals(COBieUtility.COBieNA));
		String valueAsString = COBieUtility.COBieNA;
	...
	public static IfcProperty propertyFromCOBieStringAndUnit(String propertyValue,
			String propertyName, String description,
			String unit,IfcUnitEnum unitType)
	{
		IfcProperty property =
				propertyFromCOBieString(propertyValue,propertyName,description);
		if(!COBieUtility.isNA(unit))
		{
			IfcUnitSearchDispatcher unitSearchDispatcher =
					new IfcUnitSearchDispatcher(unitType, unit);
