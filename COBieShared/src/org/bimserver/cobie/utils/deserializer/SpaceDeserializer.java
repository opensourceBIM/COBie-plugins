...
{
	private static final IfcInternalOrExternalEnum DEFAULT_SPACE_INTERNAL_EXTERNAL 
	...
	protected static final String baseQuantitiesPsetName = "BaseQuantities";
			...
			if (spaces != null)
			{
				for (SpaceType space : spaces.getSpaceArray())
				{
					...
					if (!model.containsSpace(name))
					{
						...
						List<IfcClassificationReference> classifications = classificationReferenceFromSpace(space);
						...
						IfcElementQuantity psetBaseQuantities = SpaceDeserializer
								.baseQuantitiesPropertySetFromSpace(space,ownerHistory);
	...
	private void setSpaceObjectPlacement(SpaceType cobieSpace, IfcSpace ifcSpace)
	{
		try
		{
			IfcLocalPlacement floorPlacement =
					ifcCommonHandler.getGeometryHandler().getFloorLocalPlacement(cobieSpace.getFloorName());
			IfcLocalPlacement spacePlacement =
					ifcCommonHandler.getGeometryHandler().newDefaultLocalPlacement(floorPlacement);
	...
	public  List<IfcClassificationReference> classificationReferenceFromSpace(SpaceType cobieSpace)
	{
	...
	public static IfcElementQuantity baseQuantitiesPropertySetFromSpace
										(SpaceType cobieSpace,IfcOwnerHistory ownerHistory)
	{		
	...
	public static IfcQuantityArea netFloorAreaFromSpace(SpaceType cobieSpace)
	{
		...
		{
		//THIS PRODUCES "BAD" IFC	qArea.setAreaValueAsString(COBieUtility.COBieNA);
		}
	...
	private static IfcPropertySingleValue usableHeightPropertyFromSpace(SpaceType cobieSpace) 
	{
		...
		catch (Exception e)
		{
			//IfcLengthMeasure lengthMeasure =
				//	Ifc2x3tc1Factory.eINSTANCE.createIfcLengthMeasure();
			//lengthMeasure.setWrappedValueAsString(COBieUtility.COBieNA);
