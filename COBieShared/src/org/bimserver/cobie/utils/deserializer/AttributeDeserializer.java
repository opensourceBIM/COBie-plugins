...
public class AttributeDeserializer 
{
	...
	private boolean shouldAddAttribute(AttributeType attribute)
	{
		...
		boolean isSpaceSheet =
				COBieUtility.CobieSheetName.Space.name().equalsIgnoreCase(sheetName);
		boolean isBaseQuantities =
				SpaceDeserializer.baseQuantitiesPsetName.equalsIgnoreCase(pSetName);
	...
	private boolean attributeHasExistingPropertySet(AttributeType attribute)
	{
		return (getAttributeExistingPropertySetOid(attribute)!=NULL_OID);
	}
	...
	private long getAttributeExistingPropertySetOid(AttributeType attribute)
	{
		...
		else
		{
			...
			{
				AttributeType tmpAttribute = keyedAttribute.getCobieLiteElement();
				if(
						attributeSheet.equalsIgnoreCase(tmpAttribute.getSheetName()) &&
								attributeRow.equalsIgnoreCase(tmpAttribute.getRowName()) &&
	...
	private long assignPropertySetToObject(IfcPropertySet pSet,
			AttributeType attribute,long relatedObjectOid) throws Exception
	{
			...
			if (ideObj instanceof IfcObject)
			{
				...
				propertySetOid = this.ifcCommonHandler.getPropertySetHandler().addPropertiesAndPropertySetToObject(ifcObj, pSet, false);
			}
			else if (ideObj instanceof IfcTypeObject)
			{
				IfcTypeObject ifcType = (IfcTypeObject) ideObj;
				propertySetOid = this.ifcCommonHandler.getPropertySetHandler().addPropertiesAndPropertySetToTypeObject(ifcType, pSet, false);
			}
	...
	private IfcPropertySet getPropertySetAsFilledBySoftware(AttributeType attribute) throws Exception
	{
		...
		IfcOwnerHistory ownerHistory = ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(
				createdBy, createdOn, attribute.getExtSystem());
	...
	private IfcPropertySet getPropertySetAsFilledByPerson(
			AttributeType attribute) throws Exception
	{
		String createdBy;
		Calendar createdOn;
		IfcPropertySet pSet;
	...
	public  IfcPropertySet newPropertySetFromAttribute(AttributeType attribute)
	{
		...
		if(!COBieUtility.isNA(attribute.getCategory()))
		{
			List<IfcClassificationReference> classificationReferences = 
					ifcCommonHandler.getClassificationHandler().classificationReferencesFromString(attribute.getCategory());
	...
	public IfcProperty propertyFromAttribute(AttributeType attribute) throws Exception
	{
		...
		if (PropertyUtility.isCandidateEnumeration(valStr,allowedVals))
		{
			try
			{
				newProperty =  enumeratedValueFromString(valStr, allowedVals, attribute.getName(),
					attribute.getDescription());
			}
			catch(Exception e)
			{
				newProperty = PropertyUtility.propertyFromCOBieString(valStr,
	...
	{
		this.attributesAddedToPropertySetOid = attributesAddedToOid;
	}

	private  IfcPropertyEnumeratedValue enumeratedValueFromString(String cobieValueString, String cobieAllowedValsString,
