...
public class SpareDeserializer
{
	...
	private final String SPARE_PROPERTY_SET_NAME=String.format(PropertySetHandler.fs_COBiePropertySetName, COBieUtility.CobieSheetName.Spare.name());
	...
	public SpareDeserializer(COBieIfcModel model, IfcCommonHandler ifcCommonHandler,
			...
			try
			{
				if (spares != null)
				{
					for (SpareType spare : spares.getSpareArray())
					{
						...
						{
							IfcConstructionProductResource newSpare =
									constructionProductResourceFromSpare(spare);
							IfcPropertySet sparePropertySet =
									sparePropertySetFromSpare(spare);
							...
							List<IfcClassificationReference> classificationReferences =
									ifcCommonHandler.getClassificationHandler().classificationReferencesFromString(spare.getCategory());
							for(IfcClassificationReference classificationReference:classificationReferences)
								ifcCommonHandler.getClassificationHandler().addClassificationReferenceToObject
								(classificationReference, newSpare);
							if(model.containsSpare(name))
							{
								 IfcRelAssignsToResource relationToType =
										 typeRelationFromSpare(spare,newSpare);
								 String relatedObjectClassName =
										 relationToType.getRelatedObjectsType().name();
								 if(relatedObjectClassName.endsWith(EXCLUDE_CLASS_NAME_STRING))
									 relatedObjectClassName = relatedObjectClassName.substring(0,relatedObjectClassName.length()-EXCLUDE_CLASS_NAME_STRING.length()-1);
	...
	private IfcConstructionProductResource constructionProductResourceFromSpare
	(SpareType spare) throws Exception
	{

		IfcConstructionProductResource newResource =
				Ifc2x3tc1Factory.eINSTANCE.createIfcConstructionProductResource();
		try
		{
			newResource.setName(spare.getName());
			IfcOwnerHistory ownerHistory =
					ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(spare.getCreatedBy(),
							spare.getCreatedOn(), spare.getExtSystem());
			newResource.setOwnerHistory(ownerHistory);
		
			IfcGloballyUniqueId guid =
					ifcCommonHandler.getGuidHandler().guidFromExternalIdentifier(spare.getExtIdentifier());
	...
	{
		IfcPropertySet spareProperties =
		...
		IfcPropertySingleValue supplier =
				supplierPropertyFromSpare(spare);
		IfcPropertySingleValue partNumber =
				partNumberPropertyFromSpare(spare);
		IfcPropertySingleValue setNumber =
				setNumberPropertyFromSpare(spare);
	...
	{
		IfcPropertySingleValue supplierProperty =
	...
	{
		IfcPropertySingleValue partNumberProperty =
				Ifc2x3tc1Factory.eINSTANCE.createIfcPropertySingleValue();
	...
	{
		IfcPropertySingleValue setNumberProperty =
				Ifc2x3tc1Factory.eINSTANCE.createIfcPropertySingleValue();
	...
	private IfcRelAssignsToResource typeRelationFromSpare(SpareType spare,
			IfcConstructionProductResource ifcResource)
	{
		IfcRelAssignsToResource spareAssignsRelation =
				Ifc2x3tc1Factory.eINSTANCE.createIfcRelAssignsToResource();
		
		String typeName = spare.getTypeName();
		if(model.containsType(typeName))
		{
			long typeOid =
					model.getTypeOid(typeName);
			IfcTypeObject typeObj = (IfcTypeObject)
					model.get(typeOid);
	...
	private String assignsToResourceNameFromSpareAndType(SpareType spare,
