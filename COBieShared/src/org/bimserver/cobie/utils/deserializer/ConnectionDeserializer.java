...
import org.bimserver.shared.cobie.COBieUtility;
import org.bimserver.shared.cobie.ConnectionBetweenSensorAndControllerType;
public class ConnectionDeserializer 
{
	...
	public boolean isConnectionBAMie(ConnectionType connection)
	{
		...
		if(isAComponentConnection(sheetName, componentNameOne, componentNameTwo))
		{
			...
			if(ComponentDeserializer.isIfcProductBAMie(componentOne) ||
					ComponentDeserializer.isIfcProductBAMie(componentTwo))
			{
	...
	public void deserializeBAMieConnections(List<DocumentType> bamieDocuments)
	{
		for(ConnectionType bamieConnection:bamieConnections)
		{
			if(connectsControllerAndSensor(bamieConnection))
			{
				try
				{
					DocumentType sensorDocument =
							findMatchingSensorDocument(bamieDocuments,bamieConnection);
	...
	private void deserializeBAMieConnection(ConnectionType bamieConnection,
			DocumentType sensorDocument)
	{
		List<IfcClassificationReference> classificationReferences =
				ifcCommonHandler.getClassificationHandler().classificationReferencesFromString(sensorDocument.getDirectory());
		
		IfcOwnerHistory ownerHistory =
				ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(bamieConnection.getCreatedBy(),bamieConnection.getCreatedOn(),
						bamieConnection.getExtSystem());
		ConnectionBetweenSensorAndControllerType sensorControllerConnection =
				new ConnectionBetweenSensorAndControllerType(bamieConnection.schemaType(),bamieConnection, model);
		IfcProduct controller = sensorControllerConnection.getController();
		IfcDistributionElement virtualController =
				createVirtualController(sensorControllerConnection);
	...
	{
		IfcRelAggregates aggregatesRelationship =
				COBieUtility.ifcFactory.createIfcRelAggregates();
	...
	private String getRelAggregatesControllersName(
			IfcProduct controller,
			IfcProduct childController)
	{
	...
	private IfcDistributionElement createVirtualController(
			ConnectionBetweenSensorAndControllerType sensorControllerConnection)
	{
		IfcDistributionElement virtualController =
				COBieUtility.ifcFactory.createIfcDistributionElement();
		...
		IfcRelDefinesByType definesByType = getVirtualControllerTypeRelationship(
				sensorControllerConnection, virtualController, controllerName);
	...
	private IfcRelDefinesByType getVirtualControllerTypeRelationship(
			ConnectionBetweenSensorAndControllerType sensorControllerConnection,
			IfcDistributionElement virtualController, String controllerName)
	{
		IfcControllerType controllerType = getVirtualControllerType(virtualController,sensorControllerConnection);
		IfcRelDefinesByType definesByType =
				COBieUtility.ifcFactory.createIfcRelDefinesByType();
	...
	private IfcControllerType getVirtualControllerType(
			IfcDistributionElement virtualController,ConnectionBetweenSensorAndControllerType sensorControllerConnection)
	{
		IfcControllerType controllerType =
				COBieUtility.ifcFactory.createIfcControllerType();
		String name = VIRTUAL_CONTROLLER_TYPE_PREFIX +
				VIRTUAL_CONTROLLER_TYPE_MIDDLE_NAME+virtualController.getName();
	...
	private String getVirtualControllerName(
			ConnectionBetweenSensorAndControllerType sensorControllerConnection)
	{
	...
	private boolean isAComponentConnection(String sheetName, String rowName1,
			String rowName2)
	{
		return isAComponentConnection(sheetName,rowName1,rowName2,this.model);
	}
	...
	{
		return connectsControllerAndSensor(connection,this.model);
	}
		...
		if(isAComponentConnection(sheetName, rowName1, rowName2,model))
			{
				...
				IfcControllerType controllerType =
						ComponentDeserializer.getControllerTypeInformationFromProduct(product1);
	...
	private void handleTypicalConnection(ConnectionType connection)
	{
		try
		{
	...
	private IfcRelConnects relConnectsFromConnection(ConnectionType connection)
	{
		...
		if(isAComponentConnection(connection.getSheetName(),connection.getRowName1(),connection.getRowName2()))
		{
			...
			{
				relConnects = COBieUtility.ifcFactory.createIfcRelConnectsPorts();
			}
			else if(row1Component instanceof IfcDistributionElement || 
					row2Component instanceof IfcDistributionElement)
			{
				relConnects = COBieUtility.ifcFactory.createIfcRelConnectsPortToElement();
				
			}
			else
			{
				relConnects = COBieUtility.ifcFactory.createIfcRelConnectsElements();
			}
		}
	...
	private void handleConnectsComponents(ConnectionType connection,IfcRelConnects relConn) throws Exception
	{
	...
	private void handleRelConnectsElements(ConnectionType connection,
	...
	private void handleRelConnectsPortToElement(ConnectionType connection,
			IfcRelConnectsPortToElement connectsPortsToElement)
	{
		String compName1 = connection.getRowName1();
		String compName2 = connection.getRowName2();
		String portName = "";
		...
		IfcDistributionPort thePort = distributionPortByName(portName,theDistributionElement,connection.getCreatedBy(),connection.getCreatedOn(),connection.getExtSystem());
	...
	private void handleRelConnectsPorts(ConnectionType connection,
			IfcRelConnectsPorts connectsPorts)
	{
		...
		IfcDistributionPort distPort1 = distributionPortByName(portName1,distElement1,connection.getCreatedBy(),connection.getCreatedOn(),connection.getExtSystem());
		IfcDistributionPort distPort2 = distributionPortByName(portName2,distElement2,connection.getCreatedBy(),connection.getCreatedOn(),connection.getExtSystem());
	...
	public static IfcRelConnectsPorts relConnectsPortsBetweenTwoElements(IfcElement element1, 
			IfcElement element2,COBieIfcModel cobieModel)
	{
	...
	private static boolean portIsInEitherElement(IfcElement element1,
			IfcElement element2, IfcElement port1Element)
	{
		return (element1.getGlobalId().isSetWrappedValue() && port1Element.getGlobalId().isSetWrappedValue() &&
				element1.getGlobalId().getWrappedValue().equals(port1Element.getGlobalId().getWrappedValue())) ||
				(element2.getGlobalId().isSetWrappedValue() && port1Element.getGlobalId().isSetWrappedValue() &&
						element2.getGlobalId().getWrappedValue().equals(port1Element.getGlobalId().getWrappedValue()));
	}
	...
	private IfcOwnerHistory ownerHistoryFromConnection(ConnectionType connection)
	{
		IfcOwnerHistory ownerHistory = this
				.ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(connection.getCreatedBy(), connection.getCreatedOn(),connection.getExtSystem());
		return ownerHistory;
	}
	
	private IfcDistributionPort distributionPortByName(String name,IfcDistributionElement product,String createdBy,Calendar createdOn,String extSystem)
	{
		IfcDistributionPort distPort = Ifc2x3tc1Factory.eINSTANCE.createIfcDistributionPort();
		distPort.setName(name);
		IfcOwnerHistory ownerHistory = ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(createdBy, createdOn, extSystem);
		...
		IfcRelConnectsPortToElement connectsPortToElement =
				relConnectsPortFromPortAndElement(distPort,product,ownerHistory,distPortOid);
