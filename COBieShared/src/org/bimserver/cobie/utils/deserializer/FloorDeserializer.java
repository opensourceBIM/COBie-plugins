...
public class FloorDeserializer 
{
				...
				for (FloorType floor : floors.getFloorArray())
				{
					...
					if (!model.containsFloor(name))
					{
						...
						IfcOwnerHistory ownerHistory = this
								.ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(createdBy, createdOn,floor.getExtSystem());
						...
						List<IfcClassificationReference> classifications = 
								classificationReferencesFromFloor(floor);
	...
	private void setFloorLocalPlacement(IfcBuildingStorey buildingStorey,FloorType floor)
	{
		try
		{
			IfcLocalPlacement buildingPlacement =
					(IfcLocalPlacement) model.getFirstFacility().getObjectPlacement();
			IfcLocalPlacement floorPlacement =
					ifcCommonHandler.getGeometryHandler().
					newFloorLocalPlacement(floor,buildingPlacement);
	...
	public  List<IfcClassificationReference> classificationReferencesFromFloor(FloorType floor)
	{
