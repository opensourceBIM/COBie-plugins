...
public class SystemDeserializer 
{
		...
		if (systems!=null && systems.getSystemArray()!=null)
		{
			...
			for(SystemType system: systems.getSystemArray())
			{
				try
				{
					...
					if (!model.containsSystem(systemKey) && 
							this.systemKeyToComponentNames.containsKey(systemKey))
					{
						addNewSystem(system);
					}
	...
	public void populateSystemComponentRelationships(IfcSystem system, 
			ArrayList<String> componentNames,SystemType cobieSystem)
	{
		IfcRelAssignsToGroup assignsToGroup = null;
		for(String componentName : componentNames)
		{
			IfcProduct component = (IfcProduct) model.get(model.getComponentOid(componentName));
			assignsToGroup = relAssignsToGroupFromSystemAndComponent(system,component,cobieSystem);
		}
	}
	
	
	public IfcRelAssignsToGroup relAssignsToGroupFromSystemAndComponent
	(IfcSystem system,IfcProduct component,SystemType cobieSystem)
	{
		...
		else
		{
			...
			groupAssignment.setOwnerHistory(ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(cobieSystem.getCreatedBy(),
						cobieSystem.getCreatedOn(), cobieSystem.getExtSystem()));
	...
	private IfcSystem ifcSystemFromCOBieSystem(SystemType system)
	{
		...
		ifcSystem.setOwnerHistory(ifcCommonHandler.getOwnerHistoryHandler().ownerHistoryFromEmailTimestampAndApplication(system.getCreatedBy(), system.getCreatedOn(), system.getExtSystem()));
		ifcSystem.setName(Name);
		ifcSystem.setDescription(Description);
		String objectType =
				ClassificationHandler.objectTypeFromCategoryString(system.getCategory());
		...
		List<IfcClassificationReference> classRefs =
				ifcCommonHandler.getClassificationHandler().classificationReferencesFromString(system.getCategory());
