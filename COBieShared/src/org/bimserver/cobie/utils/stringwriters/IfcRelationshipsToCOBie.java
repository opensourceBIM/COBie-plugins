...
public class IfcRelationshipsToCOBie 
{
	...
	public static enum ComparisonType{Contains,EndsWith,Equals,StartsWith};
	static Map<String,String> clonePropertyMap(Map<String,String> propertyMap)
	{
	...
	public static String getHighestRankingPropertyStringFromRelDefinesList(EList<IfcRelDefines> relDefinesList,
			ArrayList<String> prioritizedSearchStrings,boolean includeRelDefinesByType)
	{
		...
		for (IfcRelDefines def : relDefinesList)
		{
			...
			{
				Map<String,String> searchHitMap =
						IfcRelationshipsToCOBie.propertyStringsFromRelDefines(def, prioritizedSearchStrings);
				String tmpHit =
						COBieUtility.cobieStringFromStringMap(searchHitMap, prioritizedSearchStrings);
	...
	public static IfcPropertyToCOBieString getHighestRankingPsetStringFromRelDefinesList(EList<IfcRelDefines> relDefinesList,
			ArrayList<String> prioritizedSearchStrings,boolean excludeNAValues)
	{
		IfcPropertyToCOBieString highestRankingSearchHit = null;
		int searchRank = -1;
		for (IfcRelDefines def : relDefinesList)
		{
			Map<String,IfcPropertyToCOBieString> searchHitMap =
					IfcRelationshipsToCOBie.psetStringsFromRelDefines(def, prioritizedSearchStrings,false);
			IfcPropertyToCOBieString tmpHit =
					COBieUtility.cobiePsetStringFromStringMap(searchHitMap, prioritizedSearchStrings);
			int previousRank = searchRank;
			if(excludeNAValues)
			{
				if(!COBieUtility.isNA(tmpHit.getValueString()))
						searchRank = COBieUtility.cobiePsetStringRankingFromStringMap(searchHitMap, prioritizedSearchStrings);
			}
			else
				searchRank = COBieUtility.cobiePsetStringRankingFromStringMap(searchHitMap, prioritizedSearchStrings);
				...
				if (!(def instanceof IfcRelDefinesByType) && !(def==rel)) 
				{
					tmpPropertyMap = IfcRelationshipsToCOBie
							.propertyStringsFromRelDefines(def,
									propertyNamesClone);
					propertyMap = IfcPropertyToCOBieString.mergePropertyStrings(
			...
			ArrayList<String> propertyNames)
	{
		...
		if (rType!=null)
		{
			...
			{
				tmpPropertyMap =
						IfcPropertySetToCOBieString.propertyStringsFromPropertySetDefinition(pSet,propertyNamesClone);
				propertyMap = IfcPropertyToCOBieString.mergePropertyStrings(tmpPropertyMap, propertyMap);
			}
...
//				}
			...
			ArrayList<String> propertyNames,boolean exclusive)
	{
		...
		{
			tmpPropertyMap =
					IfcPropertySetToCOBieString.propertyStringsFromPropertySetDefinition(pSet,propertyNamesClone,exclusive);
			propertyMap = IfcPropertyToCOBieString.mergePropertyStrings(tmpPropertyMap, propertyMap);
		}
		if (!propertyNamesClone.isEmpty())
		{
...
//			}
	...
	static public Map<String,String> propertyStringsFromTypeObject(IfcTypeObject rType, ArrayList<String> propertyNames)
	{
		...
		{
			for (IfcPropertySetDefinition pSet : pSets) {
				tmpPropertyMap = IfcPropertySetToCOBieString.propertyStringsFromPropertySetDefinition(pSet,
						propertyNamesClone);
				propertyMap = IfcPropertyToCOBieString.mergePropertyStrings(tmpPropertyMap,
						propertyMap);
				...
				if (!(def==rel)) 
				{
					tmpPropertyMap = IfcRelationshipsToCOBie
							.psetStringsFromRelDefines(def,
									propertyNamesClone,exclusive);
					propertyMap = IfcPropertySetToCOBieString.mergePsetStrings(
							tmpPropertyMap, propertyMap);
				}
			}
		}
			...
			ArrayList<String> propertyNames,boolean exclusive)
	{
		...
		for(IfcPropertySetDefinition pSet : pSets)
		{
			...
			{
				tmpPropertyMap =
						IfcPropertySetToCOBieString.psetStringsFromPropertySetDefinition(pSet,propertyNamesClone,exclusive);
				propertyMap = IfcPropertySetToCOBieString.mergePsetStrings(tmpPropertyMap, propertyMap);
			}
			catch(Exception e)
			{
	...
	(IfcTypeObject rType, ArrayList<String> propertyNames,boolean exclusive)
	{
		...
		if (!defByTypes.isEmpty())
		{
			IfcRelDefinesByType firstAssociation =
					defByTypes.get(0);
			propertyMap = IfcRelationshipsToCOBie.
					psetStringsFromRelDefines(firstAssociation, propertyNamesClone,exclusive);
		}
		else
		{
			for (IfcPropertySetDefinition pSet : pSets) {
				tmpPropertyMap = IfcPropertySetToCOBieString.psetStringsFromPropertySetDefinition(pSet,
						propertyNamesClone,exclusive);
				propertyMap = IfcPropertySetToCOBieString.mergePsetStrings(tmpPropertyMap,
						propertyMap);
			}
		}
		return propertyMap;
	...
	static public Map<String,String> UnitStringsFromRelDefines(IfcRelDefines relDefines,
			ArrayList<String> propertyNames)
	{
