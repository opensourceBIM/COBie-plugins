...
public  class IfcPropertyToCOBieString 
{
	...
	static Map<String,String> propertyStringsFromProperty(IfcProperty property, 
		...
		else if (IfcSimpleProperty.class.isInstance(property))
		{
			...
			for (String ki : keyMatchStrings) 
			{
				if (sProperty instanceof IfcPropertySingleValue) 
				{
					...
					((PropertyNameStringValueMap)propertyVals).put(ki, valStr, sProperty);
					copyPropertyNames.remove(ki);
				}
				if (sProperty instanceof IfcPropertyEnumeratedValue)
				{
					valStr = IfcPropertyToCOBieString.stringFromPropertyEnumeratedValue
	...
	static Map<String,IfcPropertyToCOBieString> psetStringsFromProperty(IfcProperty property, 
			ArrayList<String> propertyNames,boolean exclusive)
	{
		boolean testProperty = false;
		Map<String,IfcPropertyToCOBieString> propertyVals = new HashMap<String,IfcPropertyToCOBieString>();
			...
			for (String ki : keyMatchStrings) 
			{
				if (IfcPropertySingleValue.class.isInstance(sProperty)) 
				{
					IfcSingleValueToCOBieString svStr =
							IfcPropertyToCOBieString.psetSingleValueStringFromPropertySingleValue((IfcPropertySingleValue)sProperty);
				...
				if (sProperty instanceof IfcPropertyEnumeratedValue)
				{
					IfcEnumeratedValueToCOBieString enStr =
							IfcPropertyToCOBieString.psetEnumeratedValueStringFromEnumeratedValue((IfcPropertyEnumeratedValue)sProperty);
	...
	{
		IfcEnumeratedValueToCOBieString enumStr =
				IfcPropertyToCOBieString.psetEnumeratedValueStringFromEnumeratedValue(enumVal);
	...
	protected void setValueString(IfcPhysicalSimpleQuantity quantity)
	{
		...
		if (this.valueString.length()>0 && (quantityAsString==null || !quantityAsString.equals(COBIE_NA_NUMERICAL)))
		{
			this.valueString = String.format(numFormatString, tmpFloat);
		}
		else
			this.valueString = COBIE_NA_NUMERICAL;
