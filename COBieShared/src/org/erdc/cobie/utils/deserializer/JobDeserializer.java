...
import org.erdc.cobie.sheetxmldata.COBIEType.Jobs;
import org.erdc.cobie.sheetxmldata.JobType;
import org.erdc.cobie.utils.deserializer.modelhandlers.IfcCommonHandler;
import org.erdc.cobie.utils.deserializer.modelhandlers.PropertySetHandler;
 ...
 */
public class JobDeserializer extends COBieSheetXMLDataSectionDeserializer<COBIEType.Jobs, JobType, IfcTask>
{
    ...
    @Override
    protected long addObjectToModel(IfcTask ifcObject, JobType cobieSheetXMLDataElement)
    {
        // TODO Auto-generated method stub
        return model.addJob(ifcObject, cobieSheetXMLDataElement, ifcCommonHandler);
    }

    private void addPredecessorRelation(IfcTask task, IfcTypeObject matchingType, IfcTask priorTask)
    {
    ...
    {
        // TODO Auto-generated method stub
        return model.containsJob(cobieSheetXMLDataElement);
    }

    @SuppressWarnings("unchecked")
    @Override
    protected Iterable cobieSheetXMLDataSectionIterableFromCOBieSheetXMLDataSection(Jobs cobieSheetXMLDataSection)
    {
        return new ArrayList(Arrays.asList(cobieSheetXMLDataSection.getJobArray()));
    }
    ...
    private ArrayList<IfcConstructionEquipmentResource> getMatchedResources(String resourceString)
    {
        ...
        if (resourceString.contains(COBieUtility.getCOBieDelim()))
        {
            ...
            for (String resourceName : resourceSplitStrings)
            {
                try
                {
                    long resourceOid = model.getResourceOid(resourceName);
                    IfcConstructionEquipmentResource constructionEquipmentResource = (IfcConstructionEquipmentResource)model.get(resourceOid);
        ...
        } else
        {
            if (model.containsResource(resourceString))
            {
                try
                {
                    long resourceOid = model.getResourceOid(resourceString);
                    IfcConstructionEquipmentResource constructionEquipmentResource = (IfcConstructionEquipmentResource)model.get(resourceOid);
                    resources.add(constructionEquipmentResource);
                } catch (Exception ex)
                {
    ...
    private IfcTask getPriorTask(JobType job, String priorJobId)
    {
        ...
        {
            long taskOid = model.getJobOid(clonedJob);
            priorTask = (IfcTask)model.get(taskOid);
        }
        return priorTask;
    }
    ...
    @Override
    protected void initializeAndAddClassificationsToModel(IfcTask ifcObject, JobType cobieSheetXMLDataElement) throws Exception
    {
    ...
    @Override
    protected void initializeAndAddPropertySetsToModel(IfcTask ifcObject, JobType cobieSheetXMLDataElement) throws Exception
    {
    ...
    @Override
    protected IfcTask initializeIfcFromCOBieSheetXMLData(JobType cobieElement) throws Exception
    {
    ...
    @Override
    protected void intializeAndAddRelationshipsToModel(IfcTask ifcObject, JobType cobieSheetXMLDataElement) throws Exception
    {
