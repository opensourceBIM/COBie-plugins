...
import org.erdc.cobie.shared.keyedcobiesheetxml.KeyedJobType;
...
public class COBieIfcModel extends IfcModel
{
    ...
    public long add(IdEObject eObject, OidProvider<Long> oidProvider)
    {
        long oid;
    ...
    {
        long oid = (Long)ifcCommonHandler.getOidProvider().newOid(null);
        try
        {
    ...
    {
        long oid = (Long)oidProvider.newOid(null);
        try
        {
    ...
    private long addUnit(IfcUnit unit, OidProvider<Long> oidProvider)
    {
        long oid;
        oid = oidProvider.newOid(null);
        try
    ...
    private long handleContextDependentUnit(IfcContextDependentUnit unit, OidProvider oidProvider)
    {
        long oid;
    ...
    private long handleConversionBasedUnit(IfcConversionBasedUnit unit, OidProvider oidProvider, long oid)
    {
        IfcConversionBasedUnit convUnit = unit;
        if (convUnit.getConversionFactor() != null)
        {
            if (!contains(convUnit.getConversionFactor().getUnitComponent()))
            {
                addUnit(convUnit.getConversionFactor().getUnitComponent(), oidProvider);
            }
