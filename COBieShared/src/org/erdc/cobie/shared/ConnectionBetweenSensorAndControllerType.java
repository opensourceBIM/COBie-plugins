...
public class ConnectionBetweenSensorAndControllerType
{
    ...
    public static boolean portBelongsToObject(IfcPort relatedPort, IfcElement ifcElement)
    {
        return relatedPort.isSetContainedIn()
                && relatedPort.getContainedIn().getRelatedElement().getGlobalId().isSetWrappedValue()
                && relatedPort.getContainedIn().getRelatedElement().getGlobalId().getWrappedValue()
                        .equals(ifcElement.getGlobalId().getWrappedValue());
    }
